				    ...
				    int level)
{
	...
	print_symbol_table_node(n->children[0], 0, level);
	print_symbol_table_node(n->children[1], 1, level);
}
...
static void symbol_tbl_node_free(struct symbol_tbl_node *n)
{
	...
	symbol_tbl_node_free(n->children[0]);
	symbol_tbl_node_free(n->children[1]);
	symbol_free(n->sym);
	free(n);
}
				   ...
				   const char *key, size_t key_prefixlen)
{
	...
	if (tbl->data_size >= i + 2) {
		uint16_t diff = be16toh(*(uint16_t *)&node->key[i] ^
					*(uint16_t *)&key[i]);
				   ...
				   size_t ident_len)
{
	...
	for (node = tbl->root; node;) {
		...
		matchlen =
			longest_prefix_match(tbl, node, ident, key_prefixlen);
		...
		next_child = extract_child(ident, node->prefixlen);
...
int symbol_table_update(struct symbol_tbl *tbl, struct symbol *sym)
{
	...
	key_prefix = sym->ident_len * 8;

	if (key_prefix > tbl->maxprefixlen) {
		return EINVAL;
	}

	key = symbol_ident(sym);
